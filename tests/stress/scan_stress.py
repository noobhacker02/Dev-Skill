...
Payloads (fake keys, destructive commands) live base64-encoded in cases.json so this repo's own hooks
and CI gate don't flag the test data. FAIL = the scanner did the wrong thing (a hole or a false positive).
...
def run_case(case):
    name = base64.b64decode(case["name"]).decode()
    fname = base64.b64decode(case["file"]).decode()
    content = base64.b64decode(case["content"])
    d = new_repo()
